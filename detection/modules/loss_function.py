...
def heatmap_weighted_mse_loss(
    ...
    # predictions_ij = predictions[mask]
    # targets_ij = targets[mask]
    # heatmap_masked = heatmap_reduced[mask]
    # l2_norm = torch.sum(torch.square(predictions_ij-targets_ij), dim=1)
    # M = l2_norm.shape[0]
    # return torch.sum(torch.mul(heatmap_masked, l2_norm)) / M 
    inter = torch.sum(
        heatmap * torch.square(predictions - targets)
        + torch.tensor(0, dtype=float),
        dim=1, keepdim=True).reshape(b, h, w)
    mask = heatmap.reshape(b, h, w) > heatmap_threshold
    return torch.mean(inter[mask])
